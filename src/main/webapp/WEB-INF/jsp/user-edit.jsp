...
<form:form modelAttribute="userAttribute" method="POST" action="${saveUrl}">
                ...
                <select name="addressId">
                    ...
                    <c:forEach var="address" items="${addressList}">
                        <c:if test="${requestScope.addressId != null && address.id eq requestScope.addressId}">
                            <option selected
                                    value="${address.id}">${address.region} ${address.city} ${address.street} ${address.homeNumber} ${address.flatNumber} </option>
                        </c:if>
    ...
    <input type="button" onclick="location.href='${pageContext.request.contextPath}/main/user/list.html';"
           value="Cancel"/>
</form:form>
</body>
</html>